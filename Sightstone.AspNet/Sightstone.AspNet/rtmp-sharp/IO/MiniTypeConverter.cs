...
namespace RtmpSharp.IO
{
    ...
    {
        Func<object, object> Passthrough = x => x;

        MethodInfo EnumerableToArrayMethod = typeof(MiniTypeConverter).GetMethod("EnumerableToArray", BindingFlags.| BindingFlags.NonPublic);
        T[] EnumerableToArray<T>(IEnumerable enumerable) { return enumerable.Cast<T>().ToArray(); }

        ConcurrentDictionary<Type, MethodInfo> EnumerableToArrayCache = new ConcurrentDictionary<Type, MethodInfo>();
        ConcurrentDictionary<Type, AdderMethodInfo> AdderMethodCache = new ConcurrentDictionary<Type, AdderMethodInfo>();
        ...
        object ConvertObject(Type sourceType, Type targetType, object value)
        {
        ...
        Type TryGetInterfaceType(Type targetType, Type type)
        {
    ...
    class Reflection
    {
        public bool IsNullable(this Type type)
        {
            return type.IsGenericType && type.GetGenericTypeDefinition() == typeof(Nullable<>);
        }

        public bool IsConvertible(this Type type)
        {
